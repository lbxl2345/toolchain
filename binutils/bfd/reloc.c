			  ...
			  bfd_vma addend)
{
  ...
  //lbx add codes here:
  //identify data-access instructions
  printf("reloc\n");
  if(input_section->flags & SEC_CODE  && strcmp(howto -> name, "R_X86_64_PLT32") != 0 &&
    (value >= code_end || value < code_start || code_end == 0) && (value >= rodata_end || value < rodata_start || rodata_end == 0)){
    printf("gen reloc info\n");
  Elf_Internal_Rela *rela = (Elf_Internal_Rela *) malloc(sizeof(Elf_Internal_Rela));
  rela->r_offset = input_section->output_section->vma + input_section->output_offset + address;
  rela->r_addend = bfd_get_reloc_size(howto);
  rela->r_info=0;

  struct s_rela_code_ent *rela_code = (struct s_rela_code_ent *)malloc(sizeof(struct s_rela_code_ent));
  rela_code->reloc = rela;
  if(rela_code_sec.size > 0)
    rela_code_sec.cur->next = rela_code;
  else
    rela_code_sec.head = rela_code;
  rela_code_sec.cur = rela_code;
  rela_code_sec.size++;
  printf("rela_code_sec_size is %d\n", rela_code_sec.size);
}

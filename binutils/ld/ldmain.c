...
main (int argc, char **argv)
{
  ...
  //lbx add codes here
  int code_sec_count = 0;
  asection* code_secs;
  bfd *pbfd = link_info.output_bfd;
  code_secs = malloc(sizeof(asection) * pbfd->section_count);
  asection *s;
  for(s = pbfd->sections; s && s != pbfd->section_last; s = s->next){
    if(s->flags & SEC_CODE){
      code_secs[code_sec_count] = *s;
      ++code_sec_count;
    }
  }
  ...
  extern struct s_rela_code_sec rela_code_sec;
  size_t rela_ent_size = sizeof(Elf_Internal_Rela);
  size_t idx;
  for(idx = 0, rela_code_sec.cur = rela_code_sec.head; idx < rela_code_sec.size; ++idx){
    rela_code_sec.cur = rela_code_sec.cur->next;
  }
  int r_file = open(output_filename, O_RDONLY | O_BINARY);
  size_t fsize = lseek(r_file, 0 ,SEEK_END);
  lseek(r_file, 0, SEEK_SET);
  size_t new_fsize = fsize + sizeof(Elf_Internal_Rela) * rela_code_sec.size + sizeof(size_t) + strlen(".rela.text");
  size_t file_idx = fsize;
  char *string = malloc(new_fsize);
  if(read(r_file, string, fsize) == (ssize_t)fsize){
    for(idx = 0, rela_code_sec.cur = rela_code_sec.head; idx < rela_code_sec.size; ++idx){
      Elf_Internal_Rela * reloc = rela_code_sec.cur->reloc;
      memcpy(string + file_idx, (char *)reloc, rela_ent_size);
      file_idx += rela_ent_size;
      rela_code_sec.cur = rela_code_sec.cur->next;
    }
  }
  //write relocation data to file
  close(r_file);
  int w_file = open(output_filename, O_WRONLY | O_BINARY);
  printf("total size is %d\n", rela_code_sec.size);
  size_t size = rela_code_sec.size * rela_ent_size;
  printf("rela_ent_size is%d\n", rela_ent_size);
  printf("the size is %d\n", size);
  memcpy(string + file_idx, (char *)&size, sizeof(size_t));
  file_idx += sizeof(size_t);
  strcpy(string + file_idx, ".rela.text");
  size_t written =  write(w_file, string, new_fsize);
  if(!written)
    return -1;
  free(code_secs);
  close(w_file);
...
//release data-access relocation data
void rela_code_sec_free (void) {
  size_t idx;
  struct s_rela_code_ent * tmp;
  for (idx = 0, rela_code_sec.cur = rela_code_sec.head;
      idx < rela_code_sec.size; ++idx) {
    free(rela_code_sec.cur->reloc);
    tmp = rela_code_sec.cur->next;
    free(rela_code_sec.cur);
    rela_code_sec.cur = tmp;
  }
}
static char *

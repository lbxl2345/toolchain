...
//bound-guard: string compare function
...
//bound-guard: remap code and got plt
static bool _dl_remap_code_gotplt(
  int fd, ElfW(Addr) old_l_addr, struct remapInfo *remap_info) {
  
}
//bound-guard: remap code and got plt of given module
off_t _dl_remap_module (
    int fd, const char *fname,
    ElfW(Addr) old_addr ){
#ifdef ENABLE_BOUND_GUARD

  assert (fd != -1 || fname != NULL);
  if(fname != NULL) {
    fd = __open(fname, O_RDONLY);
    assert(fd != -1);
  }

  off_t current_off = lseek(fd, 0, SEEK_CUR);//get current_offset
  off_t fsize = lseek(fd, 0, SEEK_END);
  lseek(fd, 0, SEEK_SET);
  lseek(fd, fsize - 10, SEET_SET);
  char label[11] = {0};
  read(fd, label, 10);
  //check if the module has .rela.text section
  if(StrCmp(".rela.text", label) != 0) {
    if (fname != NULL)
      fd = -1;
    else
      lseek(fd, current_offset, SEET_SET);
    return 0;
  }
  struct remapInfo remap_info = {0};
  if(_dl_remap_code_gotplt(fd, old_addr, &remap_info)){
    //get map region
  }
  if (fname != NULL)
      fd = -1;
  else
    lseek(fd, current_off, SEET_SET);

  return remap_info.newCodeBase - remap_info.oldCodeBase;
#endif
}
